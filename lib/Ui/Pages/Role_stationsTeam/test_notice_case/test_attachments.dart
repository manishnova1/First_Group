...
import 'package:railpaytro/Ui/Pages/Role_stationsTeam/test_notice_case/test_payment_details.dart';
...
class TestUnpaid_attachments extends StatefulWidget {
  @override
  _TestUnpaid_attachmentsState createState() => _TestUnpaid_attachmentsState();
}

class _TestUnpaid_attachmentsState extends State<TestUnpaid_attachments> {
  ...
  _handleImageSelection(File selectedImage) {
    if (imageslist.length < 4) {
      BlocProvider.of<TestImageSubmitBloc>(context)
          .add(TestUploadImageEvent(context: context, image: selectedImage));
    }
  }
    ...
    return Scaffold(
      bottomNavigationBar: Row(
        ...
        children: [
          ...
          BlocListener<TestImageSubmitBloc, TestImageSubmitState>(
              listener: (BuildContext context, state) {
                if (state is TestImageSubmitSuccessState) {
                  // locator<DialogService>().hideLoader();
                  setState(() {
                    imageslist.add(state.img);
                  });
                } else if (state is TestImageUploadImageProgressPercentState) {
                  // locator<DialogService>()
                  //     .showLoader(dismissable: false);
                } else if (state is TestImageSubmitErrorState) {
                  locator<DialogService>().hideLoader();
                }
              },
              child: InkWell(
                  onTap: () {
                    if (imageslist.isNotEmpty) {
                      BlocProvider.of<TestImageSubmitBloc>(context)
                          .add(TestAttachmentNextButton(context: context));
                    } else {
                  ...
                  Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        ListView.separated(
                            ...
                            itemBuilder: (context, index) {
                              return ListTile(
                                ...
                                trailing: BlocListener<TestImageSubmitBloc,
                                        TestImageSubmitState>(
                                    listener: (BuildContext context, state) {
                                      if (state
                                          is TestImageDeleteSuccessState) {
                                        setState(() {
                                    ...
                                    child: InkWell(
                                      onTap: () {
                                        BlocProvider.of<TestImageSubmitBloc>(
                                                context)
                                            .add(TestImageDeleteEvent(
                                                index, imageslist));
                                      },
                                      child: Container(
                        ...
                        MediumSpace(),
                        MediumSpace(),
                        BlocBuilder<TestImageSubmitBloc, TestImageSubmitState>(
                          builder: (BuildContext context, state) {
                            if (state
                                is TestImageUploadImageProgressPercentState) {
                              return Padding(
