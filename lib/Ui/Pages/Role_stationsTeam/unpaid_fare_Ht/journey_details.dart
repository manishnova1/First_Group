...
import 'package:railpaytro/Ui/Pages/Lookup/ufn_lookup_list.dart';
import 'package:railpaytro/Ui/Utils/Colors.dart';
import 'package:railpaytro/Ui/Utils/defaultPadiing.dart';
import 'package:railpaytro/Ui/Widgets/TextWidgets.dart';
import 'package:railpaytro/bloc/UFN_HT_BLoc/journy_info_bloc.dart';
...
class JourneyDetailsHt extends StatefulWidget {
  @override
  _JourneyDetailsHtState createState() => _JourneyDetailsHtState();
}

class _JourneyDetailsHtState extends State<JourneyDetailsHt> {
  ...
  Future<void> getIssueData(BuildContext context, String reason) async {
    final result = await Navigator.push(
        context,
        MaterialPageRoute(
            builder: (context) => UfnLookupList("UFN2", "Issuing Reason", "")));
    setState(() {
  ...
  Future<void> gettravelClass(BuildContext context, String reason) async {
    final result = await Navigator.push(
        ...
        MaterialPageRoute(
            builder: (context) =>
                travelClass("UFN HT", " Select Travel Class", "")));
    setState(() {
      if (result.toString() != "null") {
  ...
  Future<void> getTravelledTo(BuildContext context) async {
    final result = await locator<NavigationService>()
        .push(StationsListScreenRoute(caseType: "UFN (HT)"));
    setState(() {
      if (result.toString() != "null") {
        travelTo = result.toString();
        if (result.toString() == travelFrom) {
          locator<ToastService>()
              .showValidationMessage(context, "Please change destination");
        }
      } else {
        travelTo = "";
      }
            ...
            onTap: () {
              if (travelFrom.isEmpty) {
                ...
                locator<ToastService>().showValidationMessage(context,
                  ...
                  selectedCard == null) {
                locator<ToastService>().showValidationMessage(context,'Please select Railcard type');
                ...
                BlocProvider.of<JournyInfoBlocHT>(context).add(
