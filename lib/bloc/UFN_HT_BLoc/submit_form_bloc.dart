...
import 'package:railpaytro/Ui/Pages/Role_stationsTeam/unpaid_fare_Ht/addition_verification.dart';
import 'package:railpaytro/Ui/Pages/Role_stationsTeam/unpaid_fare_Ht/customer_confirmation.dart';
import 'package:railpaytro/Ui/Pages/Role_stationsTeam/unpaid_fare_Ht/offender_description_face.dart';
import 'package:railpaytro/Ui/Pages/Role_stationsTeam/unpaid_fare_Ht/unpaid_fare_notice_successfull.dart';
import 'package:railpaytro/Ui/Pages/Role_stationsTeam/unpaid_fare_Ht/zero_fare_ticket_information.dart';
import 'package:railpaytro/bloc/global_bloc.dart';
import 'package:railpaytro/common/service/toast_service.dart';
import '../../Ui/Pages/Role_stationsTeam/unpaid_fare_Ht/offender_description.dart';
...
class SubmitFormBlocHTHT extends Bloc<SubmitFormEvent, SubmitFormState> {
  UfnRepo ufnRepo;

  SubmitFormBlocHTHT(this.ufnRepo) : super(SubmitFormLoadingState());
  ...
  Stream<SubmitFormState> mapEventToState(SubmitFormEvent event) async* {
    if (event is PaymentFormEvent) {
      ...
      Navigator.push(event.context!,
          MaterialPageRoute(builder: (context) => additionVerificationHT()));
      //----------------------------------------------
      ...
      Navigator.push(event.context!,
          MaterialPageRoute(builder: (context) => customerConfirmationHT()));
    }
    //------------------------------------------------
      ...
      BlocProvider.of<GlobalBloc>(event.context)
        ...
        if (element!.cASETYPECODE == 'UFN (HT)') {
          if (element.eNABLESTATTUS == "1") {
            Navigator.push(
                event.context,
                MaterialPageRoute(
                    builder: (context) => zeroFareTicketInformationHt()));
          } else {
            Map<String, dynamic> subMap =
                BlocProvider.of<AddressUfnBloc>(event.context).submitAddressMap;
            ...
            for (var e in list) {
              if (e!.ISUSED == 0 &&
                  e.ISLOCKED == 1 &&
                  e.CASE_REFERENCE_NO!.toLowerCase().contains('ht')) {
            ...
            if (caseNum.isNotEmpty) {
              ...
              Map<String, dynamic> dumpMap = {
                'action': AppConfig.SubmitUnpaidFareHTNotice,
              ...
              submitUfnhtData(subMap, event.context, true);
            } else {
...
//
    ...
    //-------------------------------------------------
      ...
      Navigator.push(event.context!,
          MaterialPageRoute(builder: (context) => OffenderDescriptionFaceHT()));
      ...
      try {
        ...
        Fluttertoast.showToast(
            msg: "Case couldn't saved, issue reported to further check, error");
      }
      ...
      try {
        ...
        if (checkInternet) {
          ...
          if (response.statusCode == 200 && data is Map) {
            Fluttertoast.showToast(
                msg: "Offender description added successfully");
        ...
        locator<DialogService>().hideLoader();
        Fluttertoast.showToast(
            msg: 'Case couldnt saved, issue reported to further check error');
      }
    }
  }
}

Future submitUfnhtData(Map<String, dynamic> bodyData, BuildContext context,
        ...
        if (error.type == DioErrorType.receiveTimeout) {
          ...
          if (bodyData['caseNum'] != null && isOnlineMode) {
            ...
            List<Map<String, dynamic>> imageMapSyncList =
                BlocProvider.of<ImageSubmitBlocHTHT>(context).imageMapList;
            ...
            Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => unpaidfareNoticeSuccessfullHt(
